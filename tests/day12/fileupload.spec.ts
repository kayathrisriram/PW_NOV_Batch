...
test("multi file upload",async({page})=>{
    await page.goto("https://testautomationpractice.blogspot.com/")
   await page.setInputFiles("#multipleFilesInput",["./tests/filesToUpload/testFile.txt","./tests/filesToUpload/testFile2.png"])
   await page.waitForTimeout(10000)
...
test.only("multi file upload-event handler",async({page})=>{
    await page.goto("https://testautomationpractice.blogspot.com/")
        const uploadFile= page.waitForEvent('filechooser')
        await page.locator("#multipleFilesInput").click()
    const upload=await uploadFile
    upload.setFiles(["./tests/filesToUpload/testFile.txt","./tests/filesToUpload/testFile2.png"])
   await page.waitForTimeout(10000)
